...
async function fetchObsessionOnly(user) {
	try {
		const { data: html } = await client.get(`https://www.last.fm/user/${user}/obsessions`);
		return parseObsessionPage(html, `https://www.last.fm/user/${user}/obsessions`);
	} catch (e) {
...
async function fetchTopTrackOnly(user) {
	try {
		...
		const url = `https://www.last.fm/user/${user}/library/tracks`;
...
async function fetchLastFmData(user, mode = 'smart') {
	...
	const url = `https://www.last.fm/user/${user}`;
