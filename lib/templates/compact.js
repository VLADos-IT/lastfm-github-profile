 ...
 * @returns {string}
 */
function compactCard({ width, height, borderRadius, padding, url, imageBase64, rawTrack, rawArtist, safeTrack, safeArtist, headerText, bgFill }) {
	...
	const showHeader = cardWidth >= 160;
	...
	const compactHeader = (() => {
		...
		if (headerText.includes('OBSESSION')) return 'LAST.FM OBS';
		if (headerText.includes('RECENT')) return 'LAST.FM REC';
		if (headerText.includes('TOP')) return 'LAST.FM TOP';
		return 'LAST.FM';
	})();
	const background = (!bgFill || bgFill === 'none') ? 'url(#showcase-bg)' : bgFill;
	...
	<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
		...
		<a xlink:href="${url}" target="_blank">
			...
			${showHeader ? `<text x="${markX}" y="30" class="header" style="text-anchor:end;" fill="#ffe4ef">${compactHeader}</text>` : ''}
			<text x="${sidePad}" y="${trackY}" class="track" style="font-size:${trackFontSize}px;" fill="#ffffff">${displayTrack}</text>
			<text x="${sidePad}" y="${artistY}" class="artist" style="font-size:${artistFontSize}px;" fill="#ffe1d2">${displayArtist}</text>
		</a>
	</svg>`;
}
